...
export function Pesquisador(props: PesquisadorProps) {
  ...
  return (
    ...
    <div className="w-auto relative " >
      ...
      <Link to={`/researcher/${props.id}/${valoresSelecionadosExport}`} target="_blank" className={`absolute z-[9]  top-6 hidden group-hover:flex cursor-pointer items-center gap-4 bg-blue-400 hover:bg-blue-500 text-white rounded-md h-[38px] w-[38px] justify-center  font-medium transition right-20  ${valoresSelecionadosExport != '' ? "right-20" : "right-6"}`}>
      ...
      {isPopUpVisible && (
        <div key={props.id} className={"elementPesquisador flex justify-center transition-all fixed top-0 right-0 pb-24 w-full h-screen bg-[#000] z-[99999] bg-opacity-75 pt-20 overflow-y-auto overflow-x-hidden "}>
          ...
          <div className=" pt-20 flex flex-col fixed items-center h-screen right-0 top-0 w-32 z-[9999999]">
            ...
            <div className="mb-6 flex flex-col justify-center items-center">
              <Link to={`/researcher/${props.id}/${valoresSelecionadosExport}`} target="_blank" className="mb-2 h-12 w-12 rounded-md bg-blue-400 items-center justify-center flex hover:bg-blue-500 cursor-pointer transition-all">
                <ArrowSquareOut size={16} className="text-white" />
              </Link>
